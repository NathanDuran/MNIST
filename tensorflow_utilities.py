...
def dataset_placeholder(input_size, num_classes, batch_size, name='dataset'):
    ...
    with tf.name_scope(name):
        ...
        dataset = tf.data.Dataset.from_tensor_slices((instance_placeholder, labels_placeholder)).batch(batch_size).repeat(1)

        # Make iterator placeholder
        iterator = dataset.make_initializable_iterator()

        return iterator, instance_placeholder, labels_placeholder