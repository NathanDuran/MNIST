...
tensorboard_path = 'tb_logs/multi_layer'
...
# display_digit(train_images, train_labels, 0)
# display_digit(train_images, train_labels, 1)
...
iterator, images_placeholder, labels_placeholder = make_dataset_placeholder(input_size, num_classes, batch_size, 'input_dataset')
images, labels = iterator.get_next()

# Define the network
hidden_1, hidden_w1 = nn_layer(images, input_size, num_hidden_nodes, activation_func=tf.nn.sigmoid, layer_name='hidden_layer_1')
hidden_2, hidden_w2 = nn_layer(hidden_1, num_hidden_nodes, num_hidden_nodes, activation_func=tf.nn.sigmoid, layer_name='hidden_layer_2')
# hidden_3, hidden_w3 = nn_layer(hidden_2, num_hidden_nodes, num_hidden_nodes, activation_func=tf.nn.sigmoid, layer_name='hidden_layer_3')
prediction, _ = nn_layer(hidden_2, num_hidden_nodes, num_classes, activation_func=tf.nn.sigmoid, layer_name='output_layer')
...
# image_op = tf.summary.image('hidden_weights', tf.reshape(hidden_w1, [-1, 28, 28, 1]))
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        while True:
            try:
                ...
                break
        if epoch % 10 == 0:
            display_weights(hidden_w1, num_hidden_nodes)
        ...
        # image_summ = sess.run(image_op)
        # train_writer.add_summary(image_summ,epoch)
